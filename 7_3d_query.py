...
pygame.display.set_caption("OpenGL 3D Object with Occlusion Query")
...
while running:
    ...
    if keys[pygame.K_p]:
        ...
        mesh.save_to_file('output/8_2_occlusion_result_office_2.txt')

    # Remove occluded faces on 'O' key press
    if keys[pygame.K_o]:
        print("occ test")
        mesh.check_occlusion()
        mesh.save_occluded_faces('output/8_3_occluded_faces_teapot_2.txt')
        mesh.remove_occluded_faces()
        mesh.save_to_file('output/8_3_output_removedData_teapot_2.txt')
